...
public interface IRepoCheckout extends JpaRepository<Checkout,Long> {
    ...
    @Query(value="select * from pandoradbout.checkout as a INNER JOIN pandoradbout.poscheckinfar as b  ON a.poscheckin_id = b.id INNER JOIN pandoradbout.pandoracheckfarmer as c ON b.pandora_check_id=c.id WHERE c.id_userfactory=:iduserfactory AND a.status=:status AND a.substatus=:substatusa ;",nativeQuery = true)
    List<Checkout> findByIdFactory( @Param("iduserfactory") Long iduserfactory, @Param("status") String status,@Param("substatusa") String substatusa);
